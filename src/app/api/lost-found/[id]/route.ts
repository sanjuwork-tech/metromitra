// PATCH /api/lost-found/[id] — update status (author only).
...
export async function PATCH(
  ...
  if (!["active", "reunited", "closed"].includes(status)) {
    return NextResponse.json(err("Invalid status"), { status: 400 });
  }
  const item = await db.lostFound.findUnique({ where: { id } });
  ...
  const updated = await db.lostFound.update({ where: { id }, data: { status } });
